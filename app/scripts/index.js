...
function buildProduct(product, id) {
    ...
    node.append("<a href='product.html?Id=" + id + "'class='btn btn-primary'>显示</a>")
    return node;
};
window.addEventListener('load', function () {
    ...
    //老子是救火队员， 专门为傻逼填坑，这个代码不知道是哪个傻逼加的
    //window.web3 = new Web3(new Web3.providers.HttpProvider("http://localhost:8545"));
                ...
                EcommerceStore.deployed().then(function (i) {
                    ...
                    i.highestBidderInfo.call(productId).then(function (f) {
                        if (f[2].toLocaleString() == '0') {
                            $("#product-status").html("竞价已经结束. 未揭标");
                        } else {
...
function displayEndHours(seconds) {
    ...
    if (days > 0) {
        return "竞标将在" + days + " 天, " + hours + ", 小时, " + minutes + " 分钟后结束";
    } else if (hours > 0) {
        return "竞标将在" + hours + " 小时, " + minutes + " 分钟后结束";
    } else if (minutes > 0) {
        return "竞标将在" + minutes + " 分钟后结束";
    } else {
        return "竞标将在" + remaining_seconds + " 秒后结束";
    }
}
